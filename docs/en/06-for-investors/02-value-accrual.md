...
2. **Retention by apps**: apps receive rebate in CREDIT (5% default) and emission via stake (if they staked). While they do not sell, they remove CREDIT from DEX circulation.
...
5. **Buyback programs**: the DAO can approve GOV buybacks with CREDIT/stable via Treasury (see `executeBuyback`, currently a v1 stub).

## Paths through which CREDIT leaves circulation

1. **Burn on payments**: main path. 95% default of each payment is burned via `BurnTracker.burnAndRecord` → `CreditToken.burnByRole`.
2. **Burn by retention** (indirect): even without burning, CREDIT held by apps/stakers/DAO is "out" of the circulating float.
...
4. **DAO-approved buybacks** via `Treasury.executeBuyback` (once DEX integration is implemented).
...
- **(A) Direct rebate**: 5% (default) of each payment. Immediate operational cash flow.
