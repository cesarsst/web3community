...
- Staking in projects (generates weight + reward in CREDIT).
- Project listing collateral (locks GOV without generating reward).
- Voting on proposals (free).

## CREDIT — elastic supply

| Aspect | Value | Source |
|---|---|---|
| Hardcoded supply cap | **Does not exist** | `CreditToken` has no cap |
| Genesis | 10,000,000 CREDIT (one-shot) | `mintGenesis` with `genesisMinted` flag |
| Genesis recipient | `Treasury` | `ignition/modules/Dao.ts` |
| Subsequent mint | Only by `MINTER_ROLE` | `CreditToken.mint` |
| Minter in production | `RewardDistributor` | grant at deploy |
| Burn | `burn`, `burnFrom` (ERC20Burnable), `burnByRole` (role-gated) | `CreditToken` inherits `ERC20Burnable` |
...
If usage is constant (burn = constant), `alpha < 1` implies emission < burn, and supply falls gradually.
