...
> ✅ **D1+D2 IMPLEMENTADOS (2026-07-11, branch `feat/wash-signal-mitigations-d1-d2`).** `FeeRouterV2.pay` agora marca `selfPayment` (pagador = owner ou appRecipient) e NÃO credita `grossVolumeOf`/`uniquePayersOf` nesse caso (split de valor intacto — conservação preservada). Novo contador `uniquePayersOf[projectId]` (+ `hasPaid`) = pagadores DISTINTOS; **UI/automação devem ler isso no lugar da soma bruta**. Evento `PaymentRouted` ganhou `bool selfPayment` no fim (ABI mudou → resync frontend `sync:contracts` + SDK `sync-abis.mjs` no merge). 377 testes, slither limpo. **Ainda PENDENTE: D3 (gate de peso mínimo no invest, fecha auto-rodada barata) + D4 (decaimento/janela no GMV, pré-req duro de automação indexada a volume).** Custo real do ataque sybil pós-mitigação em `audit/economist/sim/sybil-cost-sim.js` (D2+D3 forçam ~$100k de GOV imobilizado → deixa de compensar).
>
